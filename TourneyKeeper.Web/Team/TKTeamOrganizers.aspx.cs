...
{
    public partial class TKTeamOrganizers : TKWebPage
    {
        ...
        protected void OrganizerGridViewRowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "RemoveOrganizer")
            {
                ...
                using (var context = new TourneyKeeperEntities())
                {
                    ...
                    TKOrganizer organizer = context.TKOrganizer.SingleOrDefault(t => t.PlayerId == pid && t.TournamentId == id);

                    context.TKOrganizer.Remove(organizer);
                    context.SaveChanges();
                }

                Response.Redirect(string.Format("/Team/TKTeamOrganizers.aspx?Id={0}", id));
            }
        }
